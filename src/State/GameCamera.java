package State;

import Entity.Entity;
import Game.*;
import Tile.*;

//Camera care este axata pe jucator.
public class GameCamera {

    float xOffset, yOffset;
    private Handler handler;
    private static volatile GameCamera instance;

    public void centerOnEntity(Entity e){
        xOffset = e.getX() - handler.getWidth()/2 + e.getWidth()/2;
        yOffset = e.getY() - handler.getHeight()/2 + e.getHeight()/2;
        checkBlankSpace();
    }

    private GameCamera(Handler handler, float xOffset, float yOffset){
        this.handler = handler;
        this.xOffset = xOffset;
        this.yOffset = yOffset;
    }

    public static GameCamera getInstance(Handler handler, float xOffset, float yOffset){
        if(instance == null){
            synchronized (GameCamera.class){
                if(instance == null){
                    instance = new GameCamera(handler, xOffset, yOffset);
                }
            }
        }
        return instance;
    }

    public void checkBlankSpace(){
        if(xOffset < 0){
            xOffset = 0;
        }else if (xOffset > handler.getLevel().getWidth() * Tile.TILE_WIDTH - handler.getWidth())
            xOffset = handler.getLevel().getWidth() * Tile.TILE_WIDTH - handler.getWidth();
        if(yOffset < 0){
            yOffset = 0;
        }else if (yOffset > handler.getLevel().getHeight() * Tile.TILE_HEIGHT - handler.getHeight())
            yOffset = handler.getLevel().getHeight() * Tile.TILE_WIDTH - handler.getHeight();
    }

    public void move(Game game, float xAmt, float yAmt){
        xOffset += xAmt;
        yOffset += yAmt;
        checkBlankSpace();
    }

    public float getxOffset() {
        return xOffset;
    }

    public void setxOffset(float xOffset) {
        this.xOffset = xOffset;
        checkBlankSpace();
    }

    public float getyOffset() {
        return yOffset;
    }

    public void setyOffset(float yOffset) {
        this.yOffset = yOffset;
        checkBlankSpace();
    }
}
